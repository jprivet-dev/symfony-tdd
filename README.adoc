...
Le premier exemple est réalisé avec une classe abstraite très simple `AbstractClass`, pour présenter un test élémentaire.

Le deuxième exemple est réalisé avec la classe `AbstractRepository`, utiliser dans <<injecter-repository>>.
...
Dans le test `PHP`, il est possible d'avoir *accès par défaut à la liste des fixtures* chargées
